...
class Solution:
    def insert(self, intervals: List[List[int]], newInterval: List[int]) -> List[List[int]]:
        ...
        for i in range(len(intervals)):
            ...
            # if it smaller, append to resutl and return result + remaining valuesof intervals 
            ...
            # check if the newInterval is bigger than the current interval, if yes, append the current interval to result
                ...
                # not appending to result to check if it overlaps with follwing intervals
            # if both condition do not satisify then newInterval is overlapping with current interval
